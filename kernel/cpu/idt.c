...
static void pic_remap(uint8_t off1, uint8_t off2) {
	...
	outb(PIC1_DATA, 0x04); io_wait(); /* ICW3: master — slave on IRQ2 */
...
void irq_handler(struct isr_frame *f) {
	if (f->int_num >= 40)          /* IRQ8-15: ACK slave */
		outb(PIC2_CMD, PIC_EOI);
	outb(PIC1_CMD, PIC_EOI);       /* ACK master */
}
...
void idt_init(void) {
	...
	pic_remap(0x20, 0x28); /* IRQ0-7 → vectors 32-39, IRQ8-15 → 40-47 */
